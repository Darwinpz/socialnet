...
export default class navegation extends Component {
    ...
    async componentDidMount() {
        ...
        const response = await axios.get('http://172.30.184.204:4000/api/usuario/ver_sesion', config);
        ...
        if (response.data.mensaje) {
            ...
            this.setState({ conectado: true, id_usuario: data.id_usuario, nombre: data.nombre, link_perfil: data.link_perfil });
    ...
    ver_perfil = (e) => {
        ...
        window.location.href = "/perfil/" + this.state.id_usuario;

    }

    salir = async () => {

        const resp = await axios.get('http://172.30.184.204:4000/api/usuario/salir', config);
                            ...
                            <ul className="navbar-nav ml-auto flex-row flex justify-content-around">
                                ...
                                <li className="nav-item dropdown active">
                                    <Link className="nav-link dropdown-toggle" to="/" data-toggle="dropdown">{this.state.nombre}
                                        <img src={this.state.ruta_server + this.state.id_usuario + "/perfil/" + this.state.link_perfil} width="25" height="25" className="rounded-circle ml-2" alt="" />
                                    </Link>

...
export default class validarcuenta extends Component {
    ...
    async componentDidMount(){
        ...
        const response = await axios.get('http://192.168.0.105:4000/api/usuario/ver_sesion', config);

        if (response.data.mensaje) {

            const data = response.data.datos;

            this.setState({ conectado: true, id_usuario: data.id_usuario, email: data.email});
    ...
    onSubmit = async (e) => {
        ...
        const response = await axios.post('http://192.168.0.105:4000/api/usuario/validar_cuenta', {
    ...
    enviar_cod = async () => {
        ...
        const response = await axios.post('http://192.168.0.105:4000/api/usuario/enviar_codverificacion', {
                ...
                <div className="card w-75 mt-5" style={cardstyle}>
                    <div className="card-body text-center">
                        <h5 className="card-title">Validación de Cuenta</h5>
                            <p className="card-text">Confírmanos que te pertenece este correo electrónico.<br></br> Introduce el código del mensaje que hemos enviado a <strong>{this.state.email}</strong>.</p>
